...
function getAvgScore(student) {
 return ( student.testScores.reduce((total, testScores) => total + testScores, 0)
  / student.testScores.length)
}

function underPerformStudents(students) {
  const underPer = students.filter((std) => getAttendanceRate(std) < 80 && getAvgScore(std) < 70)
  return underPer.map((std) => {
    return {
      name: std.name,
      attendance: getAttendanceRate(std),
      testScores: getAvgScore(std)
    }
  })
}
